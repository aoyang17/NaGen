...
| 类别 | 名称 | 数学定义 | 解释 |
|---|---|---|---|
| 优化目标 | 结构新颖性 | \(\displaystyle \max_{\mathcal C}\ f_{\mathrm{nov}}(\mathcal C),\quad f_{\mathrm{nov}}:=\min_{\mathcal C_k\in\mathcal D_{\mathrm{ref}}}d\!\left(\phi(\mathcal C),\phi(\mathcal C_k)\right)\) | 最大化候选与参考集中最近结构的距离。 |
| 优化目标 | 全 UMA 凸包能 | \(\displaystyle \min_{\mathcal C}\ E_{\mathrm{hull}}^{\mathrm{UMA}}(\mathcal C)\) | 候选能量和竞争相凸包参考全部来自同一 UMA 模型、任务头和计算协议。[A8] |
