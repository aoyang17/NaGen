...
| 约束条件 | Fe 平均形式价态 | $$\displaystyle \bar z_{\mathrm{Fe}}:=\frac{3-2y-m}{n}\ge2$$ | 电中性决定 Fe 平均形式价态；容量约束通过可氧化当量隐含 $$\bar z_{\mathrm{Fe}}<4.5$$，故不重复设置上界。[A3][A4] |
...
| Appendix | A3：形式价态 | $$\displaystyle \mathrm{Na}^{+},\ \mathrm P^{5+},\ \mathrm O^{2-},\ \mathrm{Fe}^{\bar z_{\mathrm{Fe}}+},\quad \bar z_{\mathrm{Fe}}=\frac{3-2y-m}{n}\ge2$$ | 由电中性给出 Fe 平均形式价态。 |
