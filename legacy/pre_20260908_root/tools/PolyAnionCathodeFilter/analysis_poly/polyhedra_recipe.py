...
from src.analysis_poly.poly_utils import flatten
...
def polyhedra_from_distance_cutoff(central_atoms: List[Atom],
    ...
    polyhedra = []
    lattice = central_atoms[0].site.lattice
    vertex_frac_coords = [v.frac_coords for v in vertex_atoms]
    
    for c_atom in central_atoms:
        neighbors = lattice.get_points_in_sphere(vertex_frac_coords, 
                                                 c_atom.coords, 
                                                 cutoff)
        
        vertices = []
        for fcoords, dist, i, image in neighbors:
            original_vertex_atom = vertex_atoms[i]
            new_site = copy.deepcopy(original_vertex_atom.site)
            new_site.frac_coords = fcoords
            new_site.coords = lattice.get_cartesian_coords(fcoords)
            
            new_atom = Atom(original_vertex_atom.index, new_site)
            vertices.append(new_atom)
        
        poly = CoordinationPolyhedron.try_create(central_atom = c_atom, vertices = vertices, label = label)
        if poly is not None:
