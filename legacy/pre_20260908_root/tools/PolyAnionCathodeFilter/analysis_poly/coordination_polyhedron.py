...
from src.analysis_poly.symmetry_measure import symmetry_measures_from_coordination
from src.analysis_poly.orientation_parameters import cos_theta
...
class CoordinationPolyhedron:
    ...
    @classmethod
    def try_create(cls, central_atom, vertices, label = None) -> Optional['CoordinationPolyhedron']:
        # check if the points are coplanar by trying to construct the convex hull from central atom and vertices
        try:
            all_coords = [v.site.coords for v in vertices]
            all_coords.append(central_atom.site.coords) 
            
            Delaunay(all_coords) 

            return cls(central_atom, vertices, label)

        except QhullError as e:
            if "QH6154" in str(e) or "Initial simplex is flat" in str(e) \
                or "QH6214" in str(e) or "not enough points" in str(e) \
                or "QH6013" in str(e) or "input is less than" in str(e):
                return None
            else:
                raise e
    ...
    def convex_hull(self) -> ConvexHull:
        ...
        # vertex_coords = self.minimum_image_vertex_coordinates()
        # vertex_coords = np.vstack([self.central_atom.coords, vertex_coords])
        vertex_coords = [v.coords for v in self.vertices] + [self.central_atom.coords]
        return ConvexHull(vertex_coords)
